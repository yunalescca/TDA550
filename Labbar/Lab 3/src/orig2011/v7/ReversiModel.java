...
public class ReversiModel implements GameModel {
	...
	private Dimension boardSize = Constants.getGameSize();
			...
			if (canTurn(this.turn, this.cursorPos)) {
				...
				this.board[this.cursorPos.getX()][this.cursorPos.getY()] =
						(this.turn == Turn.BLACK
								? PieceColor.BLACK
								: PieceColor.WHITE);
				pcs.firePropertyChange("Score Update", false, true);
	...
	public GameTile getGameboardState(int x, int y) {
		...
		PieceColor p = board[x][y];

		if(currentPos.equals(cursorPos)){
			if(turn == Turn.BLACK){
